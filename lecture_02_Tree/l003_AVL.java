...
public static updateHeightBalance(Node node){
    int lh=-1;
    int rh=-1;
    if(node.left!=null) lh=node.left.height;
    if(node.right!=null) rh= node.right.height;
...
public static leftRotation(Node A){
  Node B= A.right;
  Node Bleft= B.left;
  B.left=A;
  A.right= Bleft;
...
public static rightRotation(Node A){
    Node B =A.left;
    Bright= B.right;
    B.right= A;
    A.left= Bright;
...
public static getRotation(Node node){
    updateHeightBalance(node);
    if(node.bal==2){
       if(node.left.bal==1) return rightRotation(node);
       else {
           node.left= leftRotation(node.left);
           return rightRotation(node);
       }
    }else if(node.bal==-2){
       if(node.right.bal==-1) return leftRotation(node);
       else{
